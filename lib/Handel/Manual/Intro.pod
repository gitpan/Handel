...
=item * Allow interchangeable schemas / work with as many diverse cart/order schemas as possible.
...
work with schemas other than the default schema.
...
appropriate schema result. This is roughly:
    ...
    sub create {
        my $self = shift;
        return $self->schema_instance->resultset($self->schema_source)->create(shift);
    };
...
can even have its own unique storage layer.

=head1 SEE ALSO

L<Handel::Manual::Schema>, L<Handel::Manual::Storage>, L<Handel::Manual::Schema>
