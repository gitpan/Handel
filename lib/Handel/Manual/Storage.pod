...
I<in this> schema. The schema associated with the item classes own storage will be untouched.

=item * The schemas database handle will have its C<HandleError> attribute set to use the local
C<process_errors> method, used for turning database errors into Handel exceptions.
...
=item * Next, a 17th, even closer blade...
