...
a few changes to what is returned from the public methods. How subclasses are configured has
...
The RETURN_AS constants and parameters are dead. Gone. Buried. That are an ex-parrot. They were a
bad idea from my inexperience at dealing with context issues in Template Toolkit. While they are still
declared in Handel::Constants, they are not used anywhere. They will go away eventually.
...
That's all there is to it. Any other Class::DBI method, like C<table>, C<has_many>, C<columns>, etc now
reside in the Handel::Cart/Order::Schema itself. For most subclasses, this will be the extent of the changes
necessary to get everything to work. If you have performed deeper magic to alter the table name, you will
